...
func (sender *Sender) handleJdCookies(handle func(ck *JdCookie)) error {
	...
	if !sender.IsAdmin || a == "" {
		...
		if !ok {
                        //sender.Reply("你尚未绑定🐶东账号，请对我说登录，登录后即可查询账户资产信息。或者前往" + Config.JDCAddress + "进行登录")
                        //return errors.New("你尚未绑定🐶东账号，请对我说登录，登录后即可查询账户资产信息。或者前往" + Config.JDCAddress + "进行登录")
			sender.Reply("你尚未绑定🐶东账号，请对我说登录，登录后即可查询账户资产信息。")
			return errors.New("你尚未绑定🐶东账号，请对我说登录，登录后即可查询账户资产信息。")
		}
	} else {
